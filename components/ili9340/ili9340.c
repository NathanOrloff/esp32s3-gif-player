...
// height:Image height
// colors:colors
void lcdDrawImage(TFT_t * dev, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t * colors) {
    if (x + width > dev->_width) return;
    if (y + height > dev->_height) return;
    ...
    if (dev->_use_frame_buffer) {
        ...
        uint16_t _y1 = y + dev->_offsety;
        uint16_t _y2 = _y1 + (height - 1);  // full height window
        ...
        if (dev->_model == 0x9340 || dev->_model == 0x9341 || dev->_model == 0x7789 || dev->_model == 0x7796) {
            ...
            spi_master_write_colors(dev, colors, (uint32_t)width * height);  // all pixels at once
        }
